...
1. 공통 준비를 수행합니다.
2. 아래 축을 각각 웹 검색해 지난 24시간(월요일 실행이면 지난 72시간)의 새 자료를 찾습니다.
...
3. 각 자료의 **원문**을 열어 확인합니다. 요약 기사만 보고 기록하지 않습니다.
   원문 접근이 안 되면 기록하지 않고 넘어갑니다.
4. 선별 기준: 하루 **3~7개**. 아래에 해당하면 기록하지 않습니다.
...
8. 기록할 자료가 없으면 파일을 만들지 않고 종료합니다.
...
5. **자료가 뒷받침하지 않는 내용을 쓰지 않습니다.** 이번 주 자료로 할 말이 없는 topic은
   손대지 않습니다. 모든 topic을 매주 갱신할 필요가 없습니다.
6. `python scripts/check-promotion-candidates.py` 를 실행하고 출력을 확인합니다.
   후보가 있으면 파일을 옮기지 말고, 커밋 메시지 본문에 후보 목록만 남깁니다.
7. 변경이 없으면 종료합니다. 있으면 `Update topics from YYYY-MM-DD week` 로 커밋하고 push합니다.
